...
// Secret key required to register an admin account (kept in one place).
const ADMIN_SECRET = process.env.ADMIN_SECRET || 'c237adminsecret';

// Validation for the admin registration form - also needs the admin key
...
// Register an ADMIN account - only works if the correct secret key is given.
// This lets a trusted person create an admin without editing the database.
app.post('/admin/register', validateAdminRegistration, (req, res) => {
    const { username, email, password, adminKey } = req.body;

    // Check the provided secret key matches
    ...
    db.query(checkSql, [email], (error, results) => {
        ...
        // Insert the new user with role = 'admin'
        const sql = `INSERT INTO users (username, email, password, role)
                     VALUES (?, ?, SHA1(?), 'admin')`;
...
app.get('/admin/equipment/requests', checkAuthenticated, checkAdmin, (req, res) => {
    ...
    db.query(sql, (error, results) => {
        ...
        res.render('admin/equipment-request', {
...
app.post('/admin/users/:id/delete', checkAuthenticated, checkAdmin, (req, res) => {
    ...
    db.query(roleSql, [userId], (error, targetRows) => {
        ...
        db.query(activeSql, [userId], (error, results) => {
            ...
            // foreign keys block the delete. Two tables reference users.user_id:
            // loans and equipment_requests. Missing either makes the final DELETE
